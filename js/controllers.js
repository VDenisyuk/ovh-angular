...
factoryApp.controller('CalcCtrl', ['$rootScope', '$scope', '$routeParams', function ($rootScope, $scope, $routeParams) {
	...
	angular.element(document).ready(function () {
	  ...
	  $('form').submit(function(e) {
	   	...
	   	var envdTax = 1+arr_envd[$("img:last", this).attr('title')]/100;
	   	...
	   	if (tech) {
			...
			if (envdTax > 1) {
				...
				if ( $("#Bonus2", this).val() ) 
				{
					$("#Cost", this).text( "$" + commaSeparateNumber((exps / Prod_Quantity * 0.8).toFixed(2))+ " ($" +commaSeparateNumber(((exps / Prod_Quantity* 0.8)*envdTax).toFixed(2))+ ")" );			
					$("#Cost2", this).text( "$" + commaSeparateNumber((exps / Prod_Quantity2 * 0.2).toFixed(2))+ " ($" +commaSeparateNumber(((exps / Prod_Quantity2* 0.2)*envdTax).toFixed(2))+ ")" );
				}
			} else {
